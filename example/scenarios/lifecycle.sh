...
wait_for "copy finished (journal row cleared, frontier seeded)" \
    "SELECT (SELECT count(*) FROM modak.copy_progress WHERE table_id = 'public.tire_pressure_logs'::regclass::oid::bigint) = 0
        AND (SELECT replicated_lsn IS NOT NULL FROM modak.cutline WHERE table_id = 'public.tire_pressure_logs'::regclass::oid::bigint)" \
    "t"
...
LEFTOVERS=$($PSQL -tA -c "
    SELECT (SELECT count(*) FROM modak.tables WHERE table_name = 'tire_pressure_logs')
         + (SELECT count(*) FROM pg_replication_slots WHERE slot_name = 'modak_slot_public_tire_pressure_logs')
         + (SELECT count(*) FROM pg_publication WHERE pubname = 'modak_pub_public_tire_pressure_logs')")
