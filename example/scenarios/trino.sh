#!/usr/bin/env bash
# Trino reads the consistent two-tier view through the modak catalog.
...
TRINO="docker compose exec -T trino trino --output-format=CSV_UNQUOTED --catalog=modak --schema=public"
...
example/ingest.sh trip_events example/datasets/trip_events/correction-deletes.jsonl modak-delete --tier-key event_time >/dev/null
example/ingest.sh trip_events example/datasets/trip_events/correction-upsert-cold.jsonl modak-upsert >/dev/null
