...
`Dockerfile.postgres` and `Dockerfile.worker` are how the published images get built: `ghcr.io/modak-labs/tierdb` (Postgres 17 with `pg_duckdb` and the `tierdb` extension baked in), `ghcr.io/modak-labs/tierdb-embedded` (same plus the console jar and its bundled runtime), and `ghcr.io/modak-labs/tierdb-worker` (the worker daemon: tiering, mirroring, compaction). The `docker` workflow pushes them on every merge to `main` (`latest`) and on version tags. Nothing here is customer-facing. A deployment pulls the published images and configures them through env vars, see the [production deployment guide](../docs/operations/production.md), it has the full list and what to change.
