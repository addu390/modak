/* TierDB console: polls the worker's JSON API and renders the fleet. */
"use strict";
...
function chart(id) {
  if (!charts[id]) {
    charts[id] = echarts.init(document.getElementById(id), "tierdb");
  }
  return charts[id];
}
...
const SNIPPETS = [
  ["Fleet status", "SELECT * FROM tierdb.status;"],
  ["Partition lifecycle", "SELECT * FROM tierdb.partitions ORDER BY table_id, tier_key_lo;"],
  ["Delta backlog", "SELECT t.schema_name || '.' || t.table_name AS tbl, count(d.*) AS backlog\nFROM tierdb.tables t LEFT JOIN tierdb.delta d USING (table_id)\nGROUP BY 1 ORDER BY 2 DESC;"],
  ["Recent operations", "SELECT op_kind, phase, lake_snapshot_id, details, updated_at\nFROM tierdb.op_log ORDER BY updated_at DESC LIMIT 20;"],
...
function initPlayground() {
  ...
  editor.setValue("SELECT * FROM tierdb.status;");
...
function histLoad() {
  try { return JSON.parse(localStorage.getItem("tierdb.sqlHistory")) || []; }
  catch (e) { return []; }
}
...
function histPush(sql) {
  ...
  localStorage.setItem("tierdb.sqlHistory", JSON.stringify(hist.slice(0, 50)));
  renderHistory();
}
